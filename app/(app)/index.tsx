...
const HomeScreen: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    if (!areaKeyword.trim() && !typeKeyword.trim()) {
      Alert.alert('Lỗi', 'Vui lòng tìm kiếm theo khu vực hoặc loại địa điểm');
      return;
    }

    try {
      console.log(
        `Searching with ${typeKeyword}, ${areaKeyword}, ${location.latitude}, ${location.longitude}, ${distance}`
      );
      ...
      if (foundPlaces.length == 0) {
        handleClearResults();
        Alert.alert('Thông báo', 'Không tìm thấy địa điểm. Thử tìm kiếm với khu vực hoặc loại địa điểm khác');
      } else {
      ...
      <GestureHandlerRootView style={styles.container}>
        ...
        <View style={styles.searchBar}>
          <TextInput
            ...
            value={areaKeyword}
            onChangeText={setAreaKeyword}
            onSubmitEditing={handleSearch}
          />
          {areaKeyword && (
            <TouchableOpacity style={styles.clearAreaKeywordBtn} onPress={() => setAreaKeyword('')}>
              <X size={24} color="#000" />
            </TouchableOpacity>
          )}
